...
describe('HTTP server public behavior', () => {
  ...
  for (const request of [
    ...
    it(`returns the JSON 404 contract for unmatched ${request.method} routes`, async () => {
  ...
  for (const request of [
    ...
    it(`rejects unauthenticated ${request.method} ${request.path.split('?')[0]}`, async () => {
      ...
      assert.deepEqual(response.json(), { error: 'Unauthorized: Invalid or expired ID token' });
    });
  }
});
